...
const GAMES_BASE = {
    ...
    'crypto-2048': {   
        ...
        diff_mult: 1.3, 
        time: 60,
        base_target: 512,  // Reduzido para 512 (era 2048)
        target_step: 256 
    }
};

// CATÁLOGO COM ESTILOS ÚNICOS
const CATALOG = {
    miners: [
...
app.post('/api/auth/register', async (req, res) => { 
    try { 
        ...
        if(!username || !password) return res.json({success:false, message: "Invalid data"}); 
...
app.post('/api/auth/login', async (req, res) => { 
    try { 
        ...
        if(!validPass) return res.json({success:false, message: "Invalid password"}); 
...
app.post('/api/buy-room', async (req, res) => { 
    ...
    if (user.rooms_unlocked >= 5) return res.json({success:false, message: "Max rooms"}); 
