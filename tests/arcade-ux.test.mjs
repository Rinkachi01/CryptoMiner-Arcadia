...
test("Arcade mostra tutorial rápido e usa o mesmo aviso nos seis jogos", async () => {
  const [packet, hash, circuit, link, sky, crypto, notice] = await Promise.all([
    ...
    readFile(new URL("../SkyDashView.tsx", import.meta.url), "utf8"),
    readFile(new URL("../app/Crypto2048View.tsx", import.meta.url), "utf8"),
  ...
  for (const source of [packet, hash, circuit, link, sky, crypto]) {
    assert.match(source, /ArcadeStartNotice/);
    assert.match(source, /describeArcadeStart/);
  }
  assert.match(packet, /6 MINIGAMES CONECTADOS/);
